...
- 選擇「適應症大類」後，產品列表上方會顯示該大類的常見有效成分。
- 統計只使用 `substance_role=active` 且 `confirmed_status=confirmed` 的成分。
- 同一成分在同一產品中只計算一次；複方產品的每個已確認有效成分分別計數。
- `preservative`、其他賦形劑與 `unknown` 不會納入排名。
- 原資料中的「細菌感染」在介面顯示為「抗菌」，底層資料值不變。
