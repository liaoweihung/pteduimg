(() => {
  ...
  const state = { index: [], formulas: {}, analysis: {}, stage4: {}, mode: 'product', query: '', category: '', subcategory: '', formula: '', legal: '', confidence: '', page: 1, chunks: new Map() };
  ...
  function app() {
    ...
    $('#tcmSubcategory').onchange = e => { state.subcategory = e.target.value; render(); };
    $('#tcmFormula').onchange = e => { state.formula = e.target.value; state.page = 1; render(); };
    $('#tcmModes').querySelectorAll('button').forEach(b=>b.onclick=()=>{state.mode=b.dataset.mode;state.query='';state.category='';state.subcategory='';state.formula='';state.page=1;render();});
  ...
  function render() {
    ...
    $('#tcmSubcategoryLabel').hidden=state.mode!=='category'||!state.category;
  ...
  function renderCategories(){ const all=state.analysis.taxonomy||[]; const levels=unique(all.map(x=>x.level_1)); const select=$('#tcmCategory'); select.innerHTML=`<option value="">請先選擇 Level 1</option>${levels.map(v=>`<option value="${esc(v)}"${v===state.category?' selected':''}>${esc(v)}</option>`).join('')}`; const subs=unique(all.filter(x=>x.level_1===state.category).map(x=>x.level_2)); const sub=$('#tcmSubcategory'); sub.innerHTML=`<option value="">請選擇 Level 2</option>${subs.map(v=>`<option value="${esc(v)}"${v===state.subcategory?' selected':''}>${esc(v)}</option>`).join('')}`; const rows=all.filter(x=>x.level_1===state.category&&(!state.subcategory||x.level_2===state.subcategory)); $('#tcmResults').innerHTML=!state.category?`<section class="me-empty">請先選擇 Level 1 適應症大類，再選擇 Level 2 細分適應症；例如「婦科」不會直接混合成單一結果頁。</section>`:`<section class="me-card me-result-title"><h2>${esc(state.category)}${state.subcategory?' → '+esc(state.subcategory):''}</h2><p>分類依官方適應症原文的保守片語規則整理，不取代官方適應症或臨床建議。</p></section>${rows.map(x=>`<section class="me-card"><h2>${esc(x.level_2)}</h2><p class="me-summary">產品 ${x.product_count} 項・推測母方 ${x.formula_count} 個</p><h3>官方適應症原文常見片語</h3><p>${esc(x.top_official_phrases)}</p><h3>推測母方組成中的常見藥材</h3><p>${esc(x.top_herbs_from_inferred_parent)}</p><p class="tcm-raw">藥材關聯為「推測母方組成 × 官方適應症文字」的分析結果，不是確定歷史源流或臨床推薦。</p></section>`).join('')||'<section class="me-empty">此 Level 1 下尚未選擇可呈現的 Level 2 細分適應症。</section>'}`; }
  ...
  async function init() {
    try {
      const [indexResponse, formulaResponse, analysisResponse, summaryResponse, herbResponse, coResponse] = await Promise.all([fetch(`${base}index.json`), fetch(`${base}formulas.json`), fetch(`${base}tcm_indication_relationship_analysis.json`), fetch(`${base}indication_taxonomy_summary.csv`), fetch(`${base}herb_indication_profiles.csv`), fetch(`${base}herb_cooccurrence.csv`)]);
      if (![indexResponse,formulaResponse,analysisResponse,summaryResponse,herbResponse,coResponse].every(r=>r.ok)) throw Error('索引、母方或關聯分析資料無法載入');
      const [index, formulas, analysis, summaryCsv, herbCsv, coCsv] = await Promise.all([indexResponse.json(), formulaResponse.json(), analysisResponse.json(), summaryResponse.text(), herbResponse.text(), coResponse.text()]);
      if (index.productCount !== 21196 || index.products?.length !== 21196 || Object.keys(formulas.formulas || {}).length !== 204) throw Error('資料筆數驗證失敗');
      state.index = index.products; state.formulas = formulas.formulas; state.analysis = analysis; state.stage4={summaryCsv,herbCsv,coCsv}; app(); render();
