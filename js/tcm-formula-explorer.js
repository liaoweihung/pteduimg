(() => {
  ...
  const state = { index: [], formulas: {}, query: '', legal: '', confidence: '', page: 1, chunks: new Map() };
  ...
  function app() {
    document.body.innerHTML = `
      ...
      <main class="me-shell" style="--me-accent:#4b7f42">
        ...
        <section class="me-query-card"><div class="me-primary"><label>搜尋商品名、藥證字號、推測母方或母方藥材<input id="tcmSearch" type="search" placeholder="例如：知柏地黃丸、衛署成製字、熟地黃"></label></div>
  ...
  async function init() {
    try {
      const [indexResponse, formulaResponse] = await Promise.all([fetch(`${base}index.json`), fetch(`${base}formulas.json`)]);
      if (!indexResponse.ok || !formulaResponse.ok) throw Error('索引或母方資料無法載入');
      const [index, formulas] = await Promise.all([indexResponse.json(), formulaResponse.json()]);
      if (index.productCount !== 21196 || index.products?.length !== 21196 || Object.keys(formulas.formulas || {}).length !== 204) throw Error('資料筆數驗證失敗');
      state.index = index.products; state.formulas = formulas.formulas; app(); render();
